...
def main() -> None:
    ...
    result = build_illustrative_states(Path("configs/rwtam/packs"))
    after_build = _hashes(BYTE_ASSERT_PATHS)
    result.tables["out_byte_stability_assert"] = _byte_assertion_rows(before, after_build)
    paths = write_illustrative_state_outputs(result, Path("var/rwtam/scenarios/illustrative_states"))
    ...
    paths["out_byte_stability_assert"] = write_illustrative_state_outputs(
        result,
        Path("var/rwtam/scenarios/illustrative_states"),
    )["out_byte_stability_assert"]
    report = write_illustrative_state_report(result, Path("do/rwtam_illustrative_states_report_20260703.md"))
    data_upgrade_report = write_data_upgrade_report(result, Path("do/rwtam_data_upgrades_wire_report_20260703.md"))
    print(report)
    ...
    for table_name in [
        ...
        "out_brazil_comparison",
        "out_brazil_absent_with_reason",
