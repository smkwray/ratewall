...
def main() -> None:
    results = build_reissuance_policy_scenarios(Path("configs/rwtam/packs"))
    paths = write_reissuance_policy_outputs(results, Path("var/rwtam/scenarios/reissuance_policy"))
    report_path = write_reissuance_policy_report(
        results,
        Path("do/rwtam_reissuance_policy_report_20260702.md"),
    )
    m0_report_path = write_reissuance_m0_fix_report(
        results,
        Path("do/rwtam_m0_reissuance_fix_report_20260702.md"),
    )
    print(report_path)
