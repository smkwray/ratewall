...
def main() -> None:
    result = build_rwpi(Path("configs/rwtam/packs"))
    paths = write_rwpi_outputs(result, Path("var/rwtam/scenarios/rwpi"))
    report_path = write_rwpi_report(result, Path("do/rwtam_rwpi_build_report_20260703.md"))
    validation_report_path = write_rwpi_validation_report(
        result,
        Path("do/rwtam_rwpi_plug_validation_report_20260704.md"),
    )
    print(report_path)
