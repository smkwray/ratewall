...
#   - no full system paths under common user-home roots in tracked-eligible files
...
for f in ${FILES[@]+"${FILES[@]}"}; do
  [[ -f "$f" ]] || continue
  user_root_re='(/U[s]ers/[A-Za-z0-9._-]+|/h[o]me/[A-Za-z0-9._-]+|[A-Za-z]:[\\/]U[s]ers)'
  if hits="$(grep -nE "$user_root_re" "$f" 2>/dev/null)"; then
...
# --- Check 3: denylisted-term mentions in tracked-eligible files (WARN) ---
# Terms are read from the untracked local file do/hygiene_denylist.txt (one
# extended-regex per line); the check is skipped when that file is absent.
warn_ai=""
DENYLIST_FILE="do/hygiene_denylist.txt"
DENY_RE=""
if [[ -f "$DENYLIST_FILE" ]]; then
  DENY_RE="$(paste -sd'|' "$DENYLIST_FILE")"
fi
for f in ${FILES[@]+"${FILES[@]}"}; do
  [[ -z "$DENY_RE" ]] && break
  [[ -f "$f" ]] || continue
  if hits="$(grep -niE "$DENY_RE" "$f" 2>/dev/null)"; then
    while IFS= read -r line; do
      [[ -z "$line" ]] && continue
      warn_ai+="${f}:${line}"$'\n'
    done <<< "$hits"
