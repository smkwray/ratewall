...
def main() -> None:
    results = build_all_distress_scenarios(Path("configs/rwtas/packs"))
    paths = write_all_distress_scenarios(results, Path("var/rwtas/scenarios"))
    sweep_path = write_crossing_shock_sweep(
        build_crossing_shock_sweep(Path("configs/rwtas/packs")),
        Path("var/rwtas/scenarios"),
    )
    print(sweep_path)
