...
OUTPUT_DIR = Path("var/rwtam/v1/dose_modes/persistent_level")
...
REPORT_PATH = Path("do/rwtam_real_bridge_report_20260702.md")
BAND = "base"
RICARDIAN = Decimal("0")
...
def _write_report(rows: list[dict[str, str]]) -> None:
    ...
    lines = [
        "# RWTAM real annual-vs-monthly bridge report 2026-07-02",
        "",
    ...
    lines.extend(
        [
            ...
            "## R3 housekeeping",
            "",
            "- 141.17-vs-137.4 identity: `141.174851276157...` is the legacy annual-core base/ricardian-0 cumulative cashflow total used in this bridge. `137.411412218165...` is the old annual-core cumulative row in `do/rwtam_monthly_core_report_20260702.md` under the transient/default wave4 comparison table, object stamp `current_default_wave4_monthly_core:N=13.5247,D_full=211.5761,RW_full=0.06392,D/scalar=0.8556`; it is not this persistent-level bridge comparator.",
            "- `curve_beta_under_persistent_stance` is registered in `configs/rwtam/packs/structural_assumptions.csv` as a tracked diagnostic limitation with zero numeric effect.",
            "- `do/dontdo.md` now carries the anti-circularity rule for bridge/reproduction checks.",
            "",
            "## Validation",
            "",
            "- `PYTHONDONTWRITEBYTECODE=1 python scripts/build_rwtam_annual_monthly_bridge.py`",
            "- `PYTHONDONTWRITEBYTECODE=1 python scripts/build_rwtam_v1.py`",
            "- `PYTHONDONTWRITEBYTECODE=1 pytest tests/test_rwtam_*.py -q`",
