...
from ratewall.rwtas import RwtasConfigError, load_config, run_rwtas
from ratewall.rwtas.contract import OUTPUT_TABLES


CONFIG_DIR = Path("configs/rwtas")


def test_rwtas_v0_fixture_headline_and_leg_gross_diagnostic() -> None:
    result = run_rwtas(load_config(CONFIG_DIR))
...
def test_rwtas_outputs_include_required_tables() -> None:
    result = run_rwtas(load_config(CONFIG_DIR))
...
def test_rwtas_t01_to_t21_invariant_matrix_passes_without_skips() -> None:
    result = run_rwtas(load_config(CONFIG_DIR))
...
def test_t22_fungibility_reward_and_avoided_interest_variants_match(
    ...
    reward = run_rwtas(load_config(reward_config))
    avoided = run_rwtas(load_config(avoided_config))
...
def test_t23_conversion_rules_reject_leg_role_or_flow_type_magnitudes(
    ...
    with pytest.raises(RwtasConfigError, match="cell only"):
...
def test_t24_ricardian_offset_is_named_generated_and_reported(
    ...
    base = run_rwtas(base_config)
    offset = run_rwtas(offset_config)
