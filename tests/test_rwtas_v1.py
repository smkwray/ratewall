...
from ratewall.rwtas.backcast import build_backcast, write_backcast_outputs
from ratewall.rwtas.scenarios import (
    build_crossing_shock_sweep,
    build_distress_scenario,
    write_distress_scenario_outputs,
)
from ratewall.rwtas.v1 import validate_bond_mtm_overlap_rows
...
import ratewall.rwtas.v1 as v1_module


PACK_DIR = Path("configs/rwtas/packs")
GOLDEN_WAVE1_DIR = Path("tests/fixtures/rwtas/golden_wave1")
GOLDEN_WAVE2_DIR = Path("tests/fixtures/rwtas/golden_wave2")
GOLDEN_WAVE3_DIR = Path("tests/fixtures/rwtas/golden_wave3")
GOLDEN_WAVE4_DIR = Path("tests/fixtures/rwtas/golden_wave4")
GOLDEN_WAVE5_DIR = Path("tests/fixtures/rwtas/golden_wave5")
GOLDEN_WAVE6_DIR = Path("tests/fixtures/rwtas/golden_wave6")
GOLDEN_WAVE6_TAX_OFF_DIR = Path("tests/fixtures/rwtas/golden_wave6_tax_off")
...
def test_t45_wave6_refreeze_preserves_tax_on_surface(default_v1_result) -> None:
    """golden_wave6 deliberately rebases the expectations-consistent curve surface."""
    result = default_v1_result
    frozen_tables = [
        "out_ratewall_rollup",
    ...
    for table_name in frozen_tables:
        ...
        expected_text = (GOLDEN_WAVE6_DIR / f"{table_name}.csv").read_text(encoding="utf-8")
...
def test_tax_off_new_construction_is_rebaselined_and_tax_layer_is_pure_extension(
    ...
    tax_off_paths = write_v1_outputs(tax_off, tmp_path / "tax_off_wave6")

    assert tax_off.rows("out_ratewall_rollup")[0]["object_version_stamp"].startswith(
        "current_default_wave6_curve_consistency"
    )
    frozen = sorted(GOLDEN_WAVE6_TAX_OFF_DIR.glob("*.csv"))
    assert frozen
...
def test_retired_annual_monthly_bridge_is_not_current_curve_gate() -> None:
    bridge_script = Path("scripts/build_rwtas_annual_monthly_bridge.py").read_text(
        encoding="utf-8"
    )
...
def test_t46_config_only_synthetic_family_routes_without_engine_name(tmp_path: Path) -> None:
    ...
    assert synthetic_family not in Path("src/ratewall/rwtas/v1.py").read_text(encoding="utf-8")
...
def test_build_rwtas_v1_writes_required_outputs(tmp_path: Path) -> None:
    result = _session_default_v1_result()
