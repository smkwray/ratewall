"""Export-pack hysteresis and nonlinear response diagnostics for RWTAM."""
...
from ratewall.rwtam.mechanisms import MIGRATION_BANDS, build_mechanism_wave, write_mechanism_wave_outputs
from ratewall.rwtam.reissuance_policy import REISSUANCE_POLICY_SCENARIOS
...
EXPERIMENT_ID = "rwtam_hysteresis_engine_loop_20260703"
OUTPUT_DIR = Path("var/rwtam/scenarios/hysteresis_engine_loop")
REPORT_PATH = Path("do/rwtam_migration_engine_loop_report_20260703.md")
...
def write_hysteresis_report(
    ...
    lines = [
        "# RWTAM migration/beta engine-loop hysteresis upgrade",
        "",
        "Date: 2026-07-03.",
    ...
    lines.extend(
        [
            ...
            "| R2 engine-run experiment | done: migration stocks and competition beta evolve inside `src/ratewall/rwtam/v1.py` monthly loop; control/treated/ablation rows are fresh `build_v1` runs from exported month-T packs |",
        ...
        rows.append(
            {
                ...
                "source_config": "engine_loop_scenario:src/ratewall/rwtam/v1.py hysteresis_state_config;src/ratewall/rwtam/mechanisms.py:MIGRATION_BANDS;do/rwtam_beta_competition_evidence_20260703.md",
            }
        )
    return rows
...
def _state_zero_row(
    ...
    return {
        ...
        "source_engine_record": "src/ratewall/rwtam/v1.py:_monthly_records",
        "source_config": "configs/rwtam/packs/opening_stocks.csv",
    }
...
def _distress_scarring_stock(
    ...
    if pulse_bp < Decimal("300"):
        return Decimal("0")
    distress = _distress_result_for_shock(Path("configs/rwtam/packs"), pulse_bp)
...
def _old_grid_path() -> Path | None:
    for path in [
        Path("var/rwtam/scenarios/hysteresis_redo/out_hysteresis_experiment.csv"),
        Path("var/rwtam/scenarios/hysteresis/out_hysteresis_experiment.csv"),
    ]:
        if path.exists():
            return path
    return None
        ...
        [
            {
                ...
                "source": "src/ratewall/rwtam/scenarios.py:FINANCIALIZATION_SCENARIO_IDS",
                "value": "0.50",
            },
            {
                "scenario_id": "high_wall_state",
                "component": "bill_heavy",
                "source": "src/ratewall/rwtam/reissuance_policy.py:REISSUANCE_POLICY_SCENARIOS",
...
def _opening_row(family: str, amount: Decimal, holder: str, issuer: str, source_id: str) -> dict[str, str]:
    return {
        ...
        "rationale": "Month-T opening pack state exported from RWTAM engine records.",
    }
    ...
    return [
        {
            ...
            "source_file": "do/rwtam_beta_competition_evidence_20260703.md",
        ...
        },
        {
            ...
            "source_file": "src/ratewall/rwtam/mechanisms.py:MIGRATION_BANDS",
    ...
    return [
        {
            ...
            "source_file": "configs/rwtam/packs/opening_stocks.csv;src/ratewall/rwtam/v1.py:_monthly_records",
    ...
    return [
        {
            ...
            "lineage_note": "treated RW minus control RW from fresh exported-pack build_v1 outputs; migration and beta effects evolved in src/ratewall/rwtam/v1.py monthly loop",
        },
        {
...
def _superseded_formula_grid_rows() -> list[dict[str, str]]:
    old_path = Path("var/rwtam/scenarios/hysteresis/out_hysteresis_experiment.csv")
