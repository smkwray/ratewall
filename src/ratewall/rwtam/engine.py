"""Deterministic V0 monthly RWTAM engine."""
...
from ratewall.rwtam.contract import (
    OUTPUT_TABLES,
    PASS_TOLERANCE,
    SECTOR_CODES,
    SECTOR_IDS,
...
from ratewall.rwtam.schemas import (
    ClaimOpening,
    ClaimTerm,
    ExposureState,
    FlowTermRule,
...
class RwtamResult:
    """Named CSV-ready output tables from one RWTAM run."""
...
def run_rwtam(config: RwtamConfig) -> RwtamResult:
    """Run the RWTAM V0 same-state baseline/shock pair."""
    ...
    if missing:
        raise AssertionError(f"missing RWTAM output tables: {sorted(missing)}")
    return RwtamResult(tables=tables)
...
def _run_manifest_rows(config: RwtamConfig) -> list[dict[str, str]]:
    pair = config.scenario_pair
    return [
        {
            "run_pair_id": pair.run_pair_id,
...
def _invariant_rows(
    ...
    return [check | {"scope": "RWTAM_V0"} for check in checks]
...
def _check_t01(config: RwtamConfig, tables: dict[str, list[dict[str, str]]]) -> dict[str, str]:
...
def _check_t08(config: RwtamConfig) -> dict[str, str]:
...
def _check_t09(config: RwtamConfig, tables: dict[str, list[dict[str, str]]]) -> dict[str, str]:
...
def _check_t11(config: RwtamConfig, tables: dict[str, list[dict[str, str]]]) -> dict[str, str]:
...
def _tau(config: RwtamConfig) -> Decimal:
    if len(config.calendar_months) != 1:
        raise ValueError("RWTAM V0 supports one month per run")
