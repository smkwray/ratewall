"""Scenario-only RWTAM diagnostics for V2b distress packs."""
...
def write_all_distress_scenarios(
    ...
    output_root: Path = Path("var/rwtam/scenarios"),
...
def build_financialization_grid(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="rwtam_financialization_") as tmp:
        tmp_root = Path(tmp)
        results: dict[str, ScenarioResult] = {
...
def write_financialization_grid_outputs(
    results: dict[str, ScenarioResult],
    output_root: Path = Path("var/rwtam/scenarios/financialization"),
...
def write_financialization_report(
    results: dict[str, ScenarioResult],
    output_path: Path = Path("do/rwtam_financialization_report_20260702.md"),
) -> Path:
    summary = next(iter(results.values())).rows("out_financialization_grid")
    ...
    lines = [
        "# RWTAM financialization scenario grid",
        "",
        "Date: 2026-07-02.",
        "Frame: scenario run output for the financialization scenario note; the hypothesis is not treated as a target.",
        "",
        "## Grid table",
        "",
    ...
    lines.extend(
        [
            ...
            "## Output locations",
            "",
            "- `var/rwtam/scenarios/financialization/out_financialization_grid.csv`",
            "- `var/rwtam/scenarios/financialization/<scenario>/out_ratewall_rollup.csv`",
            "- `var/rwtam/scenarios/financialization/<scenario>/out_financialization_gap.csv`",
            "- `var/rwtam/scenarios/financialization/<scenario>/out_financialization_credit_supply_diagnostic.csv`",
        ]
    )
...
def _phase6_credit_supply_include_flag(pack: dict[str, list[dict[str, str]]]) -> str:
    phase6 = _load_pack(Path("configs/rwtam/packs/phase6"))
...
def build_issuance_mix_scenarios(
    ...
    *,
    ...
    base_result = build_v1(pack_dir, dose_mode=dose_mode)
    with tempfile.TemporaryDirectory(prefix="rwtam_s1_") as tmp:
        tmp_root = Path(tmp)
        results: dict[str, ScenarioResult] = {
...
def write_issuance_mix_outputs(
    results: dict[str, ScenarioResult],
    output_root: Path = Path("var/rwtam/scenarios/issuance_mix"),
) -> dict[str, dict[str, Path] | Path]:
    output_root.mkdir(parents=True, exist_ok=True)
    written: dict[str, dict[str, Path] | Path] = {}
...
def build_holder_composition_scenarios(
    ...
    *,
    ...
    base_result = build_v1(pack_dir, dose_mode=dose_mode)
    with tempfile.TemporaryDirectory(prefix="rwtam_s2_") as tmp:
        tmp_root = Path(tmp)
        results: dict[str, ScenarioResult] = {
...
def write_holder_composition_outputs(
    results: dict[str, ScenarioResult],
    output_root: Path = Path("var/rwtam/scenarios/holder_composition"),
) -> dict[str, dict[str, Path] | Path]:
    output_root.mkdir(parents=True, exist_ok=True)
    written: dict[str, dict[str, Path] | Path] = {}
    for table_name in [
...
def write_rstar_wedge(rows: list[dict[str, str]], output_root: Path = Path("var/rwtam/scenarios")) -> Path:
    output_root.mkdir(parents=True, exist_ok=True)
...
def write_s1s2_wedge_report(
    *,
    ...
    output_path: Path = Path("do/rwtam_s1s2_wedge_report_20260702.md"),
) -> Path:
    ...
    lines = [
        "# RWTAM S1/S2 scenario studies + r-star wedge",
        "",
        "Date: 2026-07-02.",
    ...
    lines.extend(
        [
            ...
            "## Output locations",
            "",
            "- `var/rwtam/scenarios/issuance_mix/`",
            "- `var/rwtam/scenarios/holder_composition/`",
            "- `var/rwtam/scenarios/out_rstar_wedge.csv`",
        ]
    )
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
