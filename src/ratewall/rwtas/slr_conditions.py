"""Scenario-only SLR-exemption conditions experiment for RWTAS."""
...
EXPERIMENT_ID = "rwtas_slr_conditions_20260703"
OUTPUT_DIR = Path("var/rwtas/scenarios/slr_conditions")
REPORT_PATH = Path("do/rwtas_slr_conditions_report_20260703.md")
ABSORPTION_REGIMES = {
...
def write_slr_conditions_report(
    ...
    lines = [
        "# RWTAS SLR-exemption conditions experiment",
        "",
        "Date: 2026-07-03.",
...
def cleanup_stale_hysteresis_redo_artifacts(
    output_dir: Path = Path("var/rwtas/scenarios/hysteresis_redo"),
) -> list[Path]:
...
def _verify_dependencies(pack_dir: Path) -> None:
    from ratewall.rwtas.hysteresis import export_state_as_opening_pack
...
def _slr_yield_delta_check(
    ...
    from ratewall.rwtas.v1 import _month_index_from_label, _treasury_yield_delta_bp
...
def _build_grid(
    ...
    with tempfile.TemporaryDirectory(prefix="rwtas_slr_conditions_") as tmp:
        tmp_root = Path(tmp)
...
def _stimulus_leg(
    ...
    with tempfile.TemporaryDirectory(prefix="rwtas_slr_tp_") as tmp:
        slr_pack = Path(tmp) / "pack"
        shutil.copytree(pack_dir, slr_pack)
        _set_slr_tp_addons(slr_pack / "term_premium_response" / "rwtas_term_premium_response_bands_2026-07-02" / "parameters_term_premium.csv", tp_values)
...
def _add_tdc_deposits_by_recipient_cells(rows: list[dict[str, str]], amount: Decimal) -> None:
    splits = _read_csv_rows(Path("configs/rwtas/packs/tdc_recipient_splits.csv"))
    for row in splits:
        share = _d(row["base"])
        if share == 0:
            continue
