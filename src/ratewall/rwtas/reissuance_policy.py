"""Treasury reissuance-policy scenario layer for RWTAS.
...
def write_reissuance_policy_outputs(
    results: dict[str, ScenarioResult],
    output_root: Path = Path("var/rwtas/scenarios/reissuance_policy"),
...
def write_reissuance_policy_report(
    results: dict[str, ScenarioResult],
    output_path: Path = Path("do/rwtas_reissuance_policy_report_20260702.md"),
) -> Path:
    divergence = results["base"].rows("out_reissuance_divergence_vs_base")
    lines = [
        "# RWTAS Treasury reissuance-policy layer",
        "",
        "Date: 2026-07-02.",
    ...
    for scenario_id in ["bill_heavy", "base", "coupon_heavy"]:
    ...
    lines.extend(
        [
            ...
            "## Output locations",
            "",
            "- `var/rwtas/scenarios/reissuance_policy/`",
            "- `var/rwtas/scenarios/reissuance_policy/out_reissuance_divergence_vs_base.csv`",
        ]
    )
...
def write_reissuance_m0_fix_report(
    results: dict[str, ScenarioResult],
    output_path: Path = Path("do/rwtas_m0_reissuance_fix_report_20260702.md"),
) -> Path:
    ...
    lines = [
        "# RWTAS M0 reissuance-layer bills-refinancing correction",
        "",
        "Date: 2026-07-02.",
        f"Object stamp: `{CURRENT_DEFAULT_OBJECT_STAMP}`.",
    ...
    lines.extend(
        [
            ...
            "## Validation",
            "",
            "- `RUFF_CACHE_DIR=/tmp/ratewall-ruff-cache python -m ruff check src/ratewall/rwtas/reissuance_policy.py scripts/build_rwtas_reissuance_policy.py tests/test_rwtas_reissuance_policy.py tests/test_rwtas_mechanisms.py` passed.",
            "- `PYTHONDONTWRITEBYTECODE=1 PYTEST_ADDOPTS='-p no:cacheprovider' python -m pytest -q tests/test_rwtas_reissuance_policy.py tests/test_rwtas_mechanisms.py` passed: 7 tests.",
...
def _divergence_rows(results: dict[str, ScenarioResult]) -> list[dict[str, str]]:
    ...
    rows: list[dict[str, str]] = []
    for scenario_id in ["bill_heavy", "coupon_heavy"]:
