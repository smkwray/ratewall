"""Scenario-only mechanism wave for placeholder-first RWTAS builds."""
...
def build_mechanism_wave(pack_dir: Path = Path("configs/rwtas/packs")) -> MechanismWaveResult:
...
def write_mechanism_wave_outputs(
    ...
    output_dir: Path = Path("var/rwtas/scenarios/mechanism_wave"),
) -> dict[str, Path]:
...
def write_mechanism_wave_report(
    result: MechanismWaveResult,
    output_path: Path = Path("do/rwtas_mechanism_wave_report_20260702.md"),
) -> Path:
    ...
    lines.extend(
        [
            ...
            "## Output locations",
            "",
            "- `var/rwtas/scenarios/mechanism_wave/out_holder_stress_ledger.csv`",
            "- `var/rwtas/scenarios/mechanism_wave/out_dsr_dispersion_crossing_profile.csv`",
            "- `var/rwtas/scenarios/mechanism_wave/out_episode_response_form_comparison.csv`",
            "- `var/rwtas/scenarios/mechanism_wave/out_endogenous_financialization_migration_path.csv`",
            "- `var/rwtas/scenarios/mechanism_wave/out_inflation_overlay_diagnostic.csv`",
        ]
    )
...
def _holder_stress_ledger(
    ...
    calibrated_dir = Path("configs/rwtas/packs/holder_balance_sheet_stress")
...
def _pack_dsr_distribution_support() -> list[dict[str, str]]:
    path = _find_pack_file(
        Path("configs/rwtas/packs/cre_maturity_dsr_dispersion"),
    ...
    for row in _read_csv_rows(path):
        ...
        key = (row["instrument_family"], row["rwtas_parent_cell_or_sector"], transition)
        target = grouped.setdefault(
            key,
            {
                "family": row["instrument_family"],
                "cell_or_sector": row["rwtas_parent_cell_or_sector"],
                "transition": transition,
...
def _episode_response_rows(pack_dir: Path) -> tuple[list[dict[str, str]], list[dict[str, str]]]:
    episode_dir = Path("configs/rwtas/packs/phase6_episode_elasticities")
...
def _mechanism_invariants(
    ...
    base_again = build_v1(Path("configs/rwtas/packs"))
