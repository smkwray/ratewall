"""CSV-backed schemas and validation for the RWTAS V0 engine."""
...
from ratewall.rwtas.contract import BANNED_COLLAPSED_LABELS, SECTOR_IDS


class RwtasConfigError(ValueError):
    """Raised when RWTAS inputs violate the V0 contract."""
...
def _decimal(value: str, *, field: str) -> Decimal:
    try:
        ...
        raise RwtasConfigError(f"invalid decimal for {field}: {value!r}") from exc
...
def _bool(value: str, *, field: str) -> bool:
    ...
    if normalized in {"false", "0", "no"}:
        return False
    raise RwtasConfigError(f"invalid boolean for {field}: {value!r}")


def _read_csv(path: Path) -> tuple[list[str], list[dict[str, str]]]:
    if not path.exists():
        raise RwtasConfigError(f"missing RWTAS config table: {path}")
    with path.open(encoding="utf-8", newline="") as handle:
        reader = csv.DictReader(handle)
        if reader.fieldnames is None:
            raise RwtasConfigError(f"missing header in RWTAS config table: {path}")
        return list(reader.fieldnames), list(reader)


def _reject_banned_label(value: str, *, table: str, field: str) -> None:
    if value in BANNED_COLLAPSED_LABELS:
        raise RwtasConfigError(
...
class RwtasConfig:
    ...
    def scenario_pair(self) -> ScenarioPair:
        if len(self.scenario_pairs) != 1:
            raise RwtasConfigError("RWTAS V0 expects exactly one scenario pair")
    ...
    def ricardian_offset(self) -> Decimal:
        ...
        if "ricardian_offset" not in params:
            raise RwtasConfigError("missing named parameter: ricardian_offset")
...
def load_config(base_path: Path) -> RwtasConfig:
    """Load and validate a RWTAS V0 config directory."""
...
def _validate_config(config: RwtasConfig) -> None:
    sector_ids = tuple(sector.sector_id for sector in config.sectors)
    if sector_ids != SECTOR_IDS:
        raise RwtasConfigError(
            "cfg_sector must list exactly the eight RWTAS base sectors in order"
        )
    if not all(sector.is_base_sector for sector in config.sectors):
        raise RwtasConfigError("all cfg_sector rows must be base sectors in V0")
    ...
    if len(claims_by_id) != len(config.claim_opening_stock):
        raise RwtasConfigError("duplicate claim_id in in_claim_opening_stock")
    terms_by_claim = {term.claim_id: term for term in config.claim_terms}
    if set(terms_by_claim) != set(claims_by_id):
        raise RwtasConfigError("claim terms must match opening claim ids exactly")
    ...
    for claim in config.claim_opening_stock:
        ...
        if claim.holder_sector not in sector_id_set:
            raise RwtasConfigError(f"unknown holder sector: {claim.holder_sector}")
        if claim.issuer_sector not in sector_id_set:
            raise RwtasConfigError(f"unknown issuer sector: {claim.issuer_sector}")
        if claim.instrument not in instrument_ids:
            raise RwtasConfigError(f"unknown instrument: {claim.instrument}")
        if claim.principal_begin_bil < 0:
            raise RwtasConfigError(f"negative claim principal: {claim.claim_id}")
        if claim.holder_cell not in conversion_cells:
            raise RwtasConfigError(f"missing holder-cell conversion: {claim.holder_cell}")
        if claim.issuer_cell not in conversion_cells:
            raise RwtasConfigError(f"missing issuer-cell conversion: {claim.issuer_cell}")

    for term in config.claim_terms:
        if term.reference_rate_id not in reference_rate_ids:
            raise RwtasConfigError(f"unknown reference rate: {term.reference_rate_id}")
        if term.default_rule_id not in default_rule_ids:
            raise RwtasConfigError(f"unknown default rule: {term.default_rule_id}")
        if term.repricing_share < 0 or term.repricing_share > 1:
            raise RwtasConfigError(f"repricing_share out of range: {term.claim_id}")
    ...
    if exposure_keys != expected_exposure_keys:
        raise RwtasConfigError("exposure-state rows must match opening claim rows")
    for exposure in config.exposure_states:
        if (
            ...
            raise RwtasConfigError(f"negative exposure partition: {exposure.claim_id}")
    ...
    for rule in config.flow_term_rules:
        if rule.claim_id not in claims_by_id:
            raise RwtasConfigError(f"flow term references unknown claim: {rule.claim_id}")
        if rule.payer_sector not in sector_id_set:
            raise RwtasConfigError(f"unknown flow-term payer sector: {rule.payer_sector}")
        ...
        if rule.payer_cell not in conversion_cells:
            raise RwtasConfigError(f"unknown flow-term payer cell: {rule.payer_cell}")
    ...
    if not pair.same_state_pair_flag:
        raise RwtasConfigError("scenario pair must be same-state in V0")
        ...
        for scenario_id in scenarios:
            for reference_rate_id in reference_rate_ids:
                ...
                if key not in path_keys:
                    raise RwtasConfigError(f"missing reference path row: {key}")
    ...
    if "ricardian_offset" not in parameter_ids:
        raise RwtasConfigError("cfg_assumption_parameter must name ricardian_offset")
    ...
    if not any(rule.additive_flag and rule.exclusive_assignment_flag for rule in report_rules):
        raise RwtasConfigError("cfg_report_group_rule must define an official view")
