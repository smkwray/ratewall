"""V1 calibrated RWTAS headline and channel-report builder."""
...
PHASE6_LAYER_CONFIG = Path("configs/rwtas/phase6_waterfall_layers.csv")
PHASE6_OVERLAP_CONFIG = Path("configs/rwtas/phase6_overlap_groups.csv")
SCENARIO_AXES_CONFIG = Path("configs/rwtas/scenario_axes.csv")
MONTHS = 120
START_YEAR = 2026
START_MONTH = 1
...
class V1Result:
    """CSV-ready RWTAS V1 output tables."""
...
def _object_version_stamp(dose_mode: str, include_tax_layer: bool = True) -> str:
    if not include_tax_layer:
        if dose_mode == "transient_12m":
            return (
                "current_default_wave6_curve_consistency_tax_off:"
                "dose_mode=transient_12m,"
                "basis=expectations_consistent_term_premium"
            )
        return f"current_default_wave6_curve_consistency_tax_off:dose_mode={dose_mode}"
    if dose_mode == DEFAULT_DOSE_MODE:
        return CURRENT_DEFAULT_OBJECT_STAMP
    return f"current_default_wave6_curve_consistency:dose_mode={dose_mode},named_comparator"
...
def _output_tables(
    ...
    *,
    ...
    tables = {
        ...
        "out_bnpl_channel": _bnpl_channel(pack, records),
        "out_bnpl_share_sensitivity": _bnpl_share_sensitivity(pack, records),
        ...
        rows.append(
            {
                ...
                "source_vintage": "pre_V15_RWTAS_structural_assumption",
...
def _cashflow_leg_gross_table(
    ...
    conversion = _conversion(pack)
    for record in records:
        routes = _cashflow_routes_for_record(pack, record)
            ...
            rows.append(
                {
                    ...
                    "band_label": _band_label(str(record["band"])),
                    "ricardian_offset": _fmt(record["ricardian_offset"]),
                    "source_channel_id": "cashflow_core_global_net",
                    "exposure_id": f"cell_net:{cell}",
                    "cell_or_sector": cell,
                    "gross_flow_bil": _fmt(amount),
                    "conversion_coefficient": _fmt(coeff),
                    "converted_effect_bil": _fmt(amount * coeff),
                    "diagnostic_role": "leg_gross_reconciles_to_global_net",
                }
            )
    return rows
            ...
            rows.append(
                {
                    ...
                    "note": "bottom_up_D_is_internal_RWTAS_drag_not_a_constraint_on_V1",
                }
            )
    return rows
...
def _bnpl_derived_stock(
    ...
    float_months = assumptions.get("bnpl_incremental_float_months", {}).get(
        band, Decimal("7")
    )
    if delta_role in {"bnpl_claim", "funding_liability"}:
        return purchases * share * duration / Decimal("12")
    if delta_role in {"float_asset"}:
        return purchases * share * float_months / Decimal("12") / Decimal("2")
    if delta_role == "remove_card_float":
        return purchases * share / Decimal("12")
    return Decimal("0")
