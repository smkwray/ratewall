...
existing RWTAS rows and the accepted price-object contract, but it does not
...
from ratewall.rwtas.scenarios import ScenarioResult
...
from ratewall.rwtas.rwpi_validation import (
...
RWPI_OUTPUT_DIR = Path("var/rwtas/scenarios/rwpi")
RWPI_REPORT_PATH = Path("do/rwtas_rwpi_build_report_20260703.md")
...
def build_rwpi(pack_dir: Path = Path("configs/rwtas/packs")) -> RwpiResult:
    ...
    invariants = _invariant_rows(windows, attribution, rent, probes, plug_validation)
...
def write_rwpi_report(result: RwpiResult, output_path: Path = RWPI_REPORT_PATH) -> Path:
    ...
    lines = [
        "# RWTAS RW_pi build report",
        "",
        "Date: 2026-07-03.",
        "Scope: scenario/diagnostic-only price object; `ND_pi` is not wired into the demand headline or RW_full goldens.",
        "Binding inputs: `configs/rwtas/packs/rwpi_design_contract/`, `configs/rwtas/packs/rwpi_design_contract/`, and `project research notes (coefficient evidence)`.",
        "",
        "## Dispositions",
        "",
        "| item | disposition |",
        "| --- | --- |",
        "| design contract | copied path-preserving to `configs/rwtas/packs/rwpi_design_contract/` |",
        ...
        "| ND_pi disclosure | window rows split demand, FX, and cost columns; the demand-only-after-wall thesis row is labeled in attribution |",
    ...
    lines.extend(
        [
            ...
            "## Validation",
            "",
            "- Validation fence: RWTAS-only tests, no skipped tests, plus byte-stable existing goldens.",
            ...
            "## Output locations",
            "",
            "- `var/rwtas/scenarios/rwpi/out_rwpi_window_path.csv`",
            "- `var/rwtas/scenarios/rwpi/out_rwpi_channel_attribution.csv`",
            "- `var/rwtas/scenarios/rwpi/out_rwpi_rent_companion_path.csv`",
            "- `var/rwtas/scenarios/rwpi/out_rwpi_pce_crosswalk.csv`",
        ]
    )
...
def write_rwpi_validation_report(
    result: RwpiResult,
    output_path: Path = Path("do/rwtas_rwpi_plug_validation_report_20260704.md"),
) -> Path:
...
def _monthly_rows(
    ...
    for slack_state in SLACK_STATES:
        for band in BANDS:
            demand_total = _demand_total_pp(result, slack_state, band)
...
def _demand_total_pp(result: ScenarioResult, slack_state: str, band: str) -> Decimal:
    row = next(
    ...
    output_slope = PHILLIPS_SLOPE_U_GAP[slack_state][band] / Decimal("2")
    ...
    rows.extend(
        [
            ...
            },
            {
                ...
                "grade": "A",
                "source": "configs/rwtas/packs/rwpi_design_contract/",
    ...
    rows.append(
        {
            ...
            "units": fx["units"],
            "grade": "scenario_path",
            "source": "configs/rwtas/packs/phase6/conversion_parameters.csv",
...
def _lineage_rows() -> list[dict[str, str]]:
    return [
        {"artifact": "design_ingest", "path": "configs/rwtas/packs/rwpi_design_contract/", "role": "binding_contract"},
        {"artifact": "design_contract_copy", "path": "configs/rwtas/packs/rwpi_design_contract/", "role": "path_preserving_copy"},
        {"artifact": "coefficient_memo", "path": "project research notes (coefficient evidence)", "role": "binding_coefficients"},
        {"artifact": "m5_overlay", "path": "var/rwtas/scenarios/mechanism_wave/out_inflation_overlay_diagnostic.csv", "role": "retained_demand_wall_first_stage"},
        {"artifact": "v1_engine_rows", "path": "var/rwtas/v1/", "role": "real_emitted_rows_for_allocation_and_first_stages"},
...
def _claim_rule_index(pack: dict[str, list[dict[str, str]]] | None = None) -> dict[str, dict[str, str]]:
    if pack is None:
        rows = _read_csv_rows(Path("configs/rwtas/packs/claim_processor_rules.csv"))
    else:
